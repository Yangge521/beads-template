...
- **即时搜索**：300ms 防抖搜索，匹配名称、标签、描述，命中关键词全局高亮（所有匹配处）
- **最近浏览**：首页横滑展示最近查看的模板，最多 8 个
- **状态持久化**：筛选、排序、难度、尺寸状态提升到 App 层，导航到详情页再返回时不丢失
...
- **用量清单导出**：一键打印用量清单（含色块、色号、名称、数量、占比、合计），打印时自动隐藏交互元素
...
- **品牌筛选**：Perler / Artkal / Hama 三品牌可多选筛选，只显示拥有所选品牌编号的颜色
- **一键复制**：点击色块即可复制色号，Toast 即时反馈
- **选购指南**：品牌说明与色差提示，帮助用户选购拼豆材料

### 体验优化
- **明暗主题**：跟随系统或手动切换，首屏无闪烁（FOUC），跨标签页同步，`color-scheme` 正确切换表单控件外观
- **键盘快捷键**：`/` 聚焦搜索、`←/→` 切换模板、`Esc` 返回、`?` 打开快捷键帮助面板
- **快捷键帮助**：按 `?` 键随时查看所有可用快捷键
- **跨标签页同步**：收藏、最近浏览、主题在多标签页间实时同步
- **动态标题**：`document.title` 随路由变化，详情页显示模板名
- **打印优化**：`@media print` 隐藏交互元素，详情页可打印用量清单（色块/色号/数量/占比表格）
- **响应式布局**：适配桌面与移动端
- **无障碍**：ARIA 属性、焦点管理、焦点环、弹窗 focus trap、skip-link（按钮实现避免 hash 路由冲突）、aria-live 搜索结果与通知
- **错误边界**：ErrorBoundary 防止渲染异常导致白屏
- **性能优化**：TemplateCard 使用 React.memo + content-visibility: auto，滚动监听 rAF 节流，useMemo 派生状态避免冗余计算
...
│   ├── ShortcutHelp.tsx     # 键盘快捷键帮助面板（? 键触发）
│   ├── TemplateCard.tsx     # 模板卡片（全局高亮）
│   └── ToastContainer.tsx   # Toast 通知系统（Context + 容器）
├── context/
